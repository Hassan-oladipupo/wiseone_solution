...
angular.module('app')
    ...
    .controller('NavBarCtrl', function($rootScope, $sessionStorage, $scope, $state, configFileService) {

        var loggedInUser = $sessionStorage.wiseOneUser;

        if (loggedInUser) {
            $scope.username = loggedInUser.Username;
            $scope.role = loggedInUser.Role.Name.toUpperCase();
            $scope.location = loggedInUser.Location.Name.toUpperCase();
        }

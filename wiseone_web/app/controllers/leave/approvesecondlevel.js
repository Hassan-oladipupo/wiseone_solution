app.controller('ApproveLeaveSecondLevelCtrl', function($scope, configFileService, $modal, $sessionStorage) {
    ...
    $scope.search = function() {
        ...
        var uri = configFileService.apiHandler.retrieveLeaveRequests;
    ...
    var initLeaveList = function(leaveRequests) {
        ...
        if ($.fn.DataTable.isDataTable('#leaveList')) {
            ...
            var table = $leaveList.DataTable({
                ...
                data: leaveRequests,
                columns: [
                    { "data": "LeaveRequest.StaffName" },
                    { "data": "LeaveRequest.LeaveTypeDescription" },
                    { "data": "LeaveRequest.LeaveDaysTaken" },
                    { "data": "LeaveRequest.StaffLocation.Name" },
                    ...
                    { "data": "Approval.ApprovalStatus" },
                    {
                        ...
                        "orderable": false,
                        "data": null,
                        "defaultContent": '<i title="Approve Leave Request" class = "fa fa-check-square-o themeprimary bordered-themeprimary dt-icon"></i>'
                    }
                ],
                order: [
                    [2, "desc"]
    ...
    $scope.approveordecline = function(data) {
        ...
        var modalInstance = $modal.open({
            backdrop: false,
            keyboard: true,
            templateUrl: 'views/leave/approvesecondlevelmodal.html',
            controller: 'LeaveModalSecondLevelCtrl',
            size: 'lg',
            resolve: {
                leaveDto: function() {
                    return data;
        ...
        modalInstance.result.then(function(leaveRequest) {
            ...
            if (leaveRequest.update) {
                ...
                configFileService
                    .put(configFileService.apiHandler.secondLevelApproveLeaveRequest, leaveRequest)
                    .success(function(data, status, headers) {
                ...
                configFileService
                    .put(configFileService.apiHandler.declineLeaveRequest, leaveRequest)
                    .success(function(data, status, headers) {
                        configFileService.displayMessage('success', 'Leave Management', data);
                        $scope.loading = false;
                        $scope.search();
